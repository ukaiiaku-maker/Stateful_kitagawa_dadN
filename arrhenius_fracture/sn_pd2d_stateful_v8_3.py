...
from .stateful_peridynamics_v8_3 import StatefulPDConfig, StatefulPDPatch
...
def _load_case_checkpoint(
    ...
    *,
    ...
    """Restore a v8.3 checkpoint and reject incompatible run settings."""
    path = Path(path)
    with np.load(path, allow_pickle=False) as data:
        metadata = json.loads(str(data["metadata_json"].item()))
        if metadata.get("checkpoint_version") != _CHECKPOINT_VERSION:
            raise RuntimeError("unsupported STATEFUL_PD_V8_3 checkpoint version")
        if metadata.get("model_id") != MODEL_ID:
            raise RuntimeError("checkpoint model identifier does not match v8.3")
        if metadata.get("source_sha256") != SOURCE_SHA256:
            raise RuntimeError(
                "checkpoint source hashes do not match the active v8.3 code; "
                "resume with the original code or use a new output directory"
            )
        expected = _checkpoint_signature(args, case_name, sigma_a_MPa)
        if metadata.get("signature") != expected:
            raise RuntimeError(
                "checkpoint arguments do not match this case; use a new output directory"
            )
        ...
        restored = {
            "next_block": int(metadata["next_block"]),
            "cycles": float(metadata["cycles"]),
            "Wp_total": float(metadata["Wp_total"]),
            "root_xy": root_xy,
...
def run_case_stress(args, case_name: str, sigma_a_MPa: float):
    ...
    print(
        "STATEFUL_PD_V8_3 resolution audit: "
    ...
    if not preflight_audit["pass"]:
        message = (
            "STATEFUL_PD_V8_3 production preflight failed: "
    ...
    if args.resume and checkpoint_path.exists():
        ...
        print(
            f"STATEFUL_PD_V8_3 resumed {case_name} sigma_a={sigma_a_MPa:g} MPa "
...
def build_parser():
    ...
    p.add_argument("--out", default="runs/sn_stateful_pd_v8_3_adaptive_active_front_pilot")
...
def main(argv=None):
    ...
    print(f"STATEFUL_PD_V8_3 fatigue model: {args.fatigue_model} (preset={args.fatigue_model_preset_applied})")
    run_sweep(args)
