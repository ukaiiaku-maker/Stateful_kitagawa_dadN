...
from .stateful_peridynamics_v8_7_local_front_spacing import StatefulPDConfig, StatefulPDPatch


MODEL_ID = "SN_2D_intact_FEM_stateful_local_peridynamics_v8_7_generalized_features_local_front_spacing_fixed_geometry"
...
def _load_case_checkpoint(
    ...
    *,
    ...
    """Restore a v8.7 checkpoint and reject incompatible run settings."""
    path = Path(path)
    with np.load(path, allow_pickle=False) as data:
        metadata = json.loads(str(data["metadata_json"].item()))
        if metadata.get("checkpoint_version") != _CHECKPOINT_VERSION:
            raise RuntimeError("unsupported STATEFUL_PD_V8_7 checkpoint version")
        if metadata.get("model_id") != MODEL_ID:
            raise RuntimeError("checkpoint model identifier does not match v8.3")
        if metadata.get("source_sha256") != SOURCE_SHA256:
            raise RuntimeError(
                "checkpoint source hashes do not match the active v8.7 code; "
