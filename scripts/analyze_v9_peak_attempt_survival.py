...
CASE=ROOT/"attempt_survival_1500_1e12_v1/Peak/shielded/sigmaA_1500MPa"
OUT=ROOT/"attempt_sn_v1"


def checkpoints(path=CASE/"v9_pd_high_cycle_mode_history.json"):
    ...
    for mode in modes:
        if "H_attempt_after_segment" not in mode:
            continue
        rows.append({
            "sigma_a_MPa":1500.0,"cycles":float(mode["cycles_total"]),
...
def main():
    OUT.mkdir(parents=True,exist_ok=True);rows=checkpoints()
    ...
    for name,target in (("N10",-math.log(.9)),("N50",math.log(2.)),("N90",-math.log(.1))):
        n,left,right=invert_piecewise(rows,target)
        q.append({"sigma_a_MPa":1500.0,"quantile":name,"target_action":target,
    ...
    (OUT/"manifest.json").write_text(json.dumps({
        ...
        "source_mode_history":str(CASE/"v9_pd_high_cycle_mode_history.json"),
