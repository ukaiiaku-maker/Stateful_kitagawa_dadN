...
GATE_FIELDS = ["material_class", "option_id", "sigma_a_MPa", "N", "phase_index", "phase", "Xi",
               ...
               "admitted_length_m", "reason", "mesh_resolved"]
...
def atomic_merge_csv(path, fields, new_rows, key_fields):
    ...
    keys = {tuple(str(row[k]) for k in key_fields) for row in new_rows}
    rows = [row for row in rows if tuple(str(row[k]) for k in key_fields) not in keys]
...
def main():
    ...
    for target in targets:
        ...
        generation = save_condition_checkpoint(
            condition, args.output/"quiet_tail_checkpoints"/args.material_class/f"N_{target:.17g}",
            kernel, summary)
    ...
    atomic_merge_csv(args.output/"300K_stable_birth_survival.csv", SURVIVAL_FIELDS,
                     survival_rows, ("material_class", "N"))
