...
def completed_from_history(history, cls):
    rows=read(history)
    if not rows or float(rows[-1]["H_attempt"]) < -math.log(.1)-1e-12: return None
    return {"material_class":cls, "option_id":OPTIONS[cls], "sigma_a_MPa":float(rows[-1]["sigma_a_MPa"]),
            ...
            "N90_attempt":crossing(rows,-math.log(.1)), "source_history":str(history)}

def inventory():
    data={c:[] for c in CLASSES}
    for r in read(PEAK):
        data["Peak"].append({k:(float(v) if k in {"sigma_a_MPa","N10_attempt","N50_attempt","N90_attempt"} else v) for k,v in r.items()})
    for r in read(DBTT):
        data["DBTT"].append({k:(float(v) if k in {"sigma_a_MPa","N10_attempt","N50_attempt","N90_attempt"} else v) for k,v in r.items()})
    for cls,dirname in (("weakT","weakT"),("ceramic","ceramic")):
        found={}
        for h in (ROOT/dirname/"action").rglob("sn_stateful_pd_history.csv"):
            row=completed_from_history(h,cls)
            if row: found[row["sigma_a_MPa"]]=row
...
def plot_one(cls, rows, pathbase):
    ...
    x10=np.array([r["N10_attempt"] for r in rows]); x90=np.array([r["N90_attempt"] for r in rows])
    if len(rows)>1:
        ax.plot(x50,y,"--",lw=1.1,color=COLORS[cls],alpha=.65,label="N50 guide")
        for a,b,yy in zip(x10,x90,y): ax.plot([a,b],[yy,yy],color=COLORS[cls],lw=1.3,alpha=.65)
    ax.scatter(x10,y,marker="|",s=65,color=COLORS[cls],label="N10")
    ax.scatter(x50,y,marker=MARKERS[cls],s=46,color=COLORS[cls],edgecolor="white",linewidth=.5,label="N50")
    ax.scatter(x90,y,marker="|",s=65,color=COLORS[cls],label="N90")
    ax.set_xscale("log"); ax.set_xlabel("Cycles to elementary attempt"); ax.set_ylabel(r"Stress amplitude $\sigma_a$ (MPa)")
    status="COMPLETE ACTION CURVE" if len(rows)>=10 and gap(rows)<=1.5 else "INCOMPLETE CAMPAIGN — COMPLETED ROWS ONLY"
...
def main():
    ...
    for cls in CLASSES:
        rows=data[cls]; x=np.array([r["N50_attempt"] for r in rows]); y=np.array([r["sigma_a_MPa"] for r in rows])
        complete=len(rows)>=10 and gap(rows)<=1.5
        if len(rows)>1: ax.plot(x,y,"-" if complete else "--",color=COLORS[cls],lw=1.2,alpha=.65)
        ax.scatter(x,y,color=COLORS[cls],marker=MARKERS[cls],s=48,label=f"{cls}: {len(rows)} rows ({'action complete' if complete else 'incomplete'})")
    ...
    fields=["class","completed_action_rows","required_action_rows","completed_conditioned_rows","required_conditioned_rows","completed_physical_rows","required_physical_rows","completed_physical_branches","required_physical_branches","lowest_completed_N50_attempt","highest_completed_N50_attempt","largest_adjacent_gap_decades","action_curve_complete","physical_matrix_complete","class_complete"]
    progress=[]
    for cls in CLASSES:
        rows=data[cls]; ac=len(rows)>=10 and gap(rows)<=1.5; pc=physical[cls]["rows"]>=10 and physical[cls]["branches"]>=40
        progress.append({"class":cls,"completed_action_rows":len(rows),"required_action_rows":10,"completed_conditioned_rows":conditioned[cls],"required_conditioned_rows":10,"completed_physical_rows":physical[cls]["rows"],"required_physical_rows":10,"completed_physical_branches":physical[cls]["branches"],"required_physical_branches":40,"lowest_completed_N50_attempt":min((r["N50_attempt"] for r in rows),default=""),"highest_completed_N50_attempt":max((r["N50_attempt"] for r in rows),default=""),"largest_adjacent_gap_decades":gap(rows),"action_curve_complete":ac,"physical_matrix_complete":pc,"class_complete":ac and pc})
    ...
    print(json.dumps({r["class"]:{"action":r["completed_action_rows"],"physical_rows":r["completed_physical_rows"],"branches":r["completed_physical_branches"]} for r in progress},indent=2))
